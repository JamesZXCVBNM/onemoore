			...
			**/
			...
			if(populate_form != '') {
				...
				_this.not('.js-ajaxified').addClass('js-ajaxified').on('change', function() {
					...
					// Reset div content
					if(jQuery(populate_form).is('div')) {
						jQuery(populate_form).empty();
						_this.attr('ajax-url-self-populate', _url);
						$.SOW.core.ajax_select.config.isDiv = true;
					}
					...
					jQuery.ajax({
						...
						success: function(data) {
							...
							/* div content */
							if(jQuery(populate_form).is('div')) {
								jQuery(populate_form).empty().append(data);
								return;
							}
		...
		ajax_process: function(_url, _method, data_params, el_target, _val, _label, _t, _callback, process_type) {
			...
			jQuery.ajax({
				...
				success: function(data) {
					...
					if($.SOW.core.ajax_select.config.isDiv === true) {
						el_target.empty().append(data);
						return;
					}
				...
				*/
				...
				jQuery('thead th.js-table-option', _t).each(function() {
					...
					jQuery('.btn-table-column-delete', _tcol).on('click', function(e) {
						...
						var _colIDDel = _tcol.parents('th').data('id');
...
;(function ($) {
	...
	$.SOW.core.input_suggest = {
			...
			if(_type == 'append') {
				...
				_this.on('keyup change', function(e) {
					...
					window.afterSearchKeyUp = setTimeout(function() {
						...
						if(_key.length >= Number(_charMin)) {
							...
							$.SOW.core.input_suggest.__suggestAjax(_url,_method,_limit,'input_search',_key,_elID,_this,_type,_container);
						} else {
				...
				_this.on('click', function(e) {
					...
					if(jQuery('#dd_'+_elID).length < 1) {
						$.SOW.core.input_suggest.__suggestAjax(_url,_method,_limit,'input_suggest',_key,_elID,_this,_type,_container);
						return;
					}
		 ...
		 *
		 **/
		__suggestAjax: function(_url,_method,_limit,_action,_key,_elID,_this,_type,_container) {
